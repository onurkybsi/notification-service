...
internal class NotificationTemplateDeletionUseCaseTest {
    ...
    @Test
    fun `should delete an existing template by given ID`() = runTest {
        // given
    ...
    @Test
    fun `should return DataNotFoundFailure when the template to delete doesn't exist by given ID`() = runTest {
        // given
        val id = TestData.notificationTemplate.id
    ...
    fun `should return UnexpectedFailure when something went unexpectedly wrong during execution`() = runTest {
        ...
        coEvery { repositoryPort.delete(id) }
            .returns(NotificationTemplateRepositoryPort.DeletionFailure.UnexpectedFailure(RuntimeException("Something went unexpectedly wrong!")).left())
