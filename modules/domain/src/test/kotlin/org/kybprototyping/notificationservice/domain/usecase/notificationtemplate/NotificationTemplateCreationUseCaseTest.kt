...
internal class NotificationTemplateCreationUseCaseTest {
    ...
    fun `should create a notification template`() = runTest {
        // given
        coEvery { repositoryPort.create(
            testInput.channel, testInput.type,
            testInput.language, testInput.subject, testInput.content) } returns 1.right()
    ...
    @Test
    fun `should return DataConflictFailure when notification template with given channel, type and language is already created`() = runTest {
        // given
        coEvery { repositoryPort.create(
            testInput.channel, testInput.type,
            testInput.language, testInput.subject, testInput.content) } returns null.right()
    ...
    fun `should return UnexpectedFailure when something went unexpectedly wrong during execution`() = runTest {
        ...
        coEvery { repositoryPort.create(
            testInput.channel, testInput.type,
            testInput.language, testInput.subject, testInput.content) } returns UnexpectedFailure("Something went unexpectedly wrong!").left()
    ...
    private companion object {
        val testInput = NotificationTemplateCreationInput(
